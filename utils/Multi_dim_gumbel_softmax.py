...
def multi_dim_softmax(logits: torch.Tensor, tau_add_one: torch.Tensor, eps: float = 1e-10) -> torch.Tensor:
    ...
    gumbels = - torch.empty_like(logits,
                                 memory_format=torch.legacy_contiguous_format).exponential_().log()  # (M,N), ~Gumbel(0,1)
