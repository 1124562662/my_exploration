...
class IntrinsicAgent(nn.Module):
    def __init__(self,envs,
        ...
        self.pseudo_ucb_net = UcbCountNet(envs, embed_dim=args.embed_dim, target=False).to(device)
        self.pseudo_ucb_optimizer = optim.Adam(self.pseudo_ucb_net.parameters(), lr=args.pseudo_ucb_optimizer_lr)
        self.pseudo_ucb_target = UcbCountNet(envs, embed_dim=args.embed_dim, target=True).to(device)
    ...
    def get_action(self,
        ...
        obs_emb = self.byol_encoder(obs_) #TODO 有问题
